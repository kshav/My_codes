...
{ 
 ...
 for(i=0;a[i]!='\0';i++)
 {
 if(temp->ptr[int(a[i])-97]!=NULL)
 temp=temp->ptr[int(a[i])-97];
 else
 break;                       
 }
 if(i<(a.size()-1)||(i==a.size()&&temp->bol==0)||(i<a.size()&&temp->info!=a[i]))
 cout<<"\nstring not exist ";
 else
 {
  int inp=0;
  ...
  else
  {
   ...
   while(pr!=root)
   {
    ...
    char d=temp->info;
    cout<<d<<" deleting\n";
    pr->ptr[int(d)-97]=NULL;
    temp->parent=NULL;
    delete (temp);
    for(int j=0;j<26;j++)
    {
    if(pr->ptr[j]!=NULL||pr->bol==1)
    {
    inp=1;
    break;
    }
