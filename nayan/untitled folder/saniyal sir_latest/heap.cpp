...
{ 
      int info;
      int priority;
      int d;    //to determine the index number of node
      node *left;
      node *right;
}*root=NULL,*p=NULL;
...
void preorder(node *ptr,int count)         //to find node equal to index count
{
      if (ptr!=NULL)
      {
      if(ptr->d==count)
...
void entry(int n,int &count)          //to insert value into a tree in binary tree form
{
...
void maxheap(int no)                       //convert the given tree into max heap
{
...
void del(int n,int &count)                //deleting the tree maintaining heap tree
{
       cout<<root->info;
       preorder(root,n);
